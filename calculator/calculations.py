...
def multiply(a: Union[float, int], b: Union[float, int]) -> float:
    ...
    Args:
        a: A number representing the first factor.
        b: A number representing the second factor.
...
def divide(a: Union[float, int], b: Union[float, int]) -> float:
    ...
    Raises:
        ZeroDivisionError: If the divisor `b` is equal to zero.

    Returns:
        A number representing the quotient of `a` divided by `b`.
    """
    if b == 0:
